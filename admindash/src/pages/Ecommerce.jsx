...
import { deleteTasks, listTasks, updateTasksStage, updateTasksToBin } from "../actions/tasksActions";
import { Dialog } from "primereact/dialog";
import { Button } from "primereact/button";
import { Message } from "primereact/message";
import { ProgressSpinner } from "primereact/progressspinner";
import { BIN_TASKS_RESET, GET_TASKS_RESET, UPDATE_TASKS_STAGE_RESET } from "../constants/tasksConstants";
...
const Ecommerce = () => {
  ...
  const {  loading: loadingUpdate,
    error: errorUpdate,
    success: successUpdate, } = tasksUpdateStage;

    const tasksBin = useSelector((state) => state.tasksBin);
    const {  loading: loadingBinUpdate,
      error: errorBinUpdate,
      success: successBinUpdate } = tasksBin;
  ...
  }, [dispatch, navigate,successUpdate,  userInfo, process, Finished, Delivered, UnFinished, successDelete, successBinUpdate]);
  ...
  return (
    <div className="mt-14">
      ...
      <div className="md:m-10 mt-24 p-4 md:p-10 bg-white rounded-3xl m-2">
       ...
       <center> <form className="w-full xl:max-w-xl max-w-lg flex relative">
            <input
              type="text"
              ...
              <tbody>
                {tasks.map((tasks) => (
                  <tr id={tasks._id}>
                    ...
                    <td>${tasks.amount}</td>
                    <td><Button label="" icon="pi pi-comment" onClick={() => setMessage(true)} />
<Dialog header="Comment" visible={message} onHide={() => setMessage(false)}
    ...
    {tasks.comment}
                    ...
                    <td>
                    {tasks.stage === 0 ? (
                        <p className="text-white bg-blue-600 text-center px-1">On Process</p>
                      ) : tasks.stage === 1 ? (
                        <p className="text-whit bg-yellow-300 text-center px-1">Finished</p>
                      ) :tasks.stage === 2 ? (
                        <p className="text-white bg-green-500 text-center px-1">Delivered</p>
                      ):<p className="text-white bg-red-600 text-center  px-1">Unfinished</p> }
                    </td>
                    <td>
                      <Button
                        label=""
        ...
        >
          <form onSubmit={() => updateTaskStage(tasks._id, stage)}>
              ...
              <div className="mt-4 flex justify-center">
                ...
                >
                  Update
                    ...
                    <td>
                    {userInfo.role ==1 ?<Button label="" icon="pi pi-delete-left" onClick={()=> binTask(tasks._id)} /> : <Button label="" icon="pi pi-delete-left" onClick={()=> deleteTask(tasks._id)} />}  
                    </td>
                  </tr>
                ))}
              </tbody>
