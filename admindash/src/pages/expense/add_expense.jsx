...
import { createNewExpense } from "../../actions/expenseActions";
...
const AddExpense = () => {
  ...
  const createExpenseItem = useSelector((state) => state.createExpenseItem);
  const {
  ...
  const submitHandler = (e) => {
    e.preventDefault();
    // DISPACTH REGISTER
    dispatch(createNewExpense(title, amount, type, date, ref));
  };

  return (
    <div className="container m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-2xl">
      {/* <!-- checkout form --> */}
      <Header category="Add" title="New Expense" />
