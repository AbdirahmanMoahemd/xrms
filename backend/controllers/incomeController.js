...
export const getTotalIncomeByTasks = expressAsync(async (req, res) => {
  try {
    let toltalIncTasks;
    const tasks = await Tasks.find({stage: 2}).sort({ createdAt: -1 });


    let toltalIncProducts;
    const items = await Sales.find({isPaid: true}).sort({ createdAt: -1 });
    ...
    let totalProductsInc = addDecimals(
      (toltalIncProducts = items.reduce((acc, item) => acc + item.price, 0))
    );
   ...
   let totalProductsIncome= kFormatter(totalProductsInc)
    ...
    res.json({ totalTasksIncome, totalProductsIncome });
  } catch (error) {
    res.json({ error: error.message });
  }
});
